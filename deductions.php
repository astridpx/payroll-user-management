								...
								?>
									<tr>
										...
										<td class="text-center">
											<button class="btn btn-sm btn-primary edit_deductions" type="button" data-id="<?php echo $row['id'] ?>" data-deduction="<?php echo $row['deduction'] ?>" data-description="<?php echo $row['description'] ?>">Edit</button>
...
<script>
	...
	$('#manage-deductions').submit(function(e) {
		...
		start_load()
		$.ajax({
			url: './services/ajax.php?action=save_deductions',
	...
	function delete_deductions($id) {
		start_load()
		$.ajax({
			url: './services/ajax.php?action=delete_deductions',
			method: 'POST',
			data: {
				id: $id
			},
